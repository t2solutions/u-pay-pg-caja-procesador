...
import com.upayments.pg.caja.api.io.soap.getcardbynumber.DataInput;

@Component(value="getCardByNumberRoute")
public class GetCardByNumberRoute extends RestRouteBuilder {

	@Override
	public void configure() throws Exception {
		
	    rest("/getCardByNumber")
	    	.post("/")
	    	.description("Servicio para invocar a WS SOAP GetCardByNumber")
	    	.type(DataInput.class)
	    	.outType(String.class)
	    	.consumes("application/json")
	    	.produces("text/plain")
	    	.to("direct:GetCardByNumber");	
	    
	    from("direct:GetCardByNumber")
	    	.handleFault()	    	
		    ...
		    .unmarshal().json(JsonLibrary.Jackson, DataInput.class)
		    .to("velocity:GetCardByNumber_Req.vm")
		    .log("HEADER IDA -> ${in.header.CamelSpringWebServiceSoapHeader}")
		    .log("BODY IDA -> ${body}")
		    .log("DESTINO: -> {{soap.remote.GetCardByNumberOp.ep}} ACCION: -> {{soap.remote.GetCardByNumberOp.action}} ")
	    	.to("spring-ws:{{soap.remote.GetCardByNumberOp.ep}}?soapAction={{soap.remote.GetCardByNumberOp.action}}")
	    	...
	    	.to("xslt:GetCardByNumber_Body_Rsp.xsl") //Por defecto, output es string
	    	.log("BODY VUELTA POST XSLT-> ${body}")  	
	    	
	    	.convertBodyTo(String.class)
	    	.to("xslt:GetCardByNumber_Aux_Rsp.xsl")
