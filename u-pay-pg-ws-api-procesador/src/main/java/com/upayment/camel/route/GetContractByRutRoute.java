...
import com.upayments.pg.caja.api.io.soap.getcontractbyrut.DataInput;

@Component(value="getContractByRutRoute")
public class GetContractByRutRoute extends RestRouteBuilder {

	@Override
	public void configure() throws Exception {
		
	    rest("/getContractByRut")
	    	.post("/")
	    	.description("Servicio para invocar a WS SOAP GetContractByRut")
	    	.type(DataInput.class)
	    	.outType(String.class)
	    	.consumes("application/json")
	    	.produces("text/plain")
	    	.to("direct:GetContractByRut");	
	    
	    from("direct:GetContractByRut")
	    	.handleFault()	    	
		    ...
		    .unmarshal().json(JsonLibrary.Jackson, DataInput.class)
		    .to("velocity:GetContractByRut_Req.vm")
		    .log("HEADER IDA -> ${in.header.CamelSpringWebServiceSoapHeader}")
		    .log("BODY IDA -> ${body}")
		    .log("DESTINO: -> {{soap.remote.GetContractByRutOp.ep}} ACCION: -> {{soap.remote.GetContractByRutOp.action}} ")
	    	.to("spring-ws:{{soap.remote.GetContractByRutOp.ep}}?soapAction={{soap.remote.GetContractByRutOp.action}}")
	    	...
	    	.to("xslt:GetContractByRut_Body_Rsp.xsl") //Por defecto, output es string
	    	.log("BODY VUELTA POST XSLT-> ${body}")  	
	    	
	    	.convertBodyTo(String.class)
	    	.to("xslt:GetContractByRut_Aux_Rsp.xsl")
