...
import com.upayments.pg.caja.api.io.soap.getcardbycontract.DataInput;

@Component(value="getCardByContractRoute")
public class GetCardByContractRoute extends RestRouteBuilder {

	@Override
	public void configure() throws Exception {
		
	    rest("/getCardByContract")
	    	.post("/")
	    	.description("Servicio para invocar a WS SOAP GetCardByContract")
	    	.type(DataInput.class)
	    	.outType(String.class)
	    	.consumes("application/json")
	    	.produces("text/plain")
	    	.to("direct:GetCardByContract");	
	    
	    from("direct:GetCardByContract")
	    	.handleFault()	    	
		    ...
		    .unmarshal().json(JsonLibrary.Jackson, DataInput.class)
		    .to("velocity:GetCardByContract_Req.vm")
		    .log("HEADER IDA -> ${in.header.CamelSpringWebServiceSoapHeader}")
		    .log("BODY IDA -> ${body}")
		    .log("DESTINO: -> {{soap.remote.GetCardByContractOp.ep}} ACCION: -> {{soap.remote.GetCardByContractOp.action}} ")
	    	.to("spring-ws:{{soap.remote.GetCardByContractOp.ep}}?soapAction={{soap.remote.GetCardByContractOp.action}}")
	    	...
	    	.to("xslt:GetCardByContract_Body_Rsp.xsl") //Por defecto, output es string
	    	.log("BODY VUELTA POST XSLT-> ${body}")  	
	    	
	    	.convertBodyTo(String.class)
	    	.to("xslt:GetCardByContract_Aux_Rsp.xsl")
