...
import com.upayments.pg.caja.api.io.soap.getcontractclassifier.DataInput;

@Component(value="getContractClassifierRoute")
public class GetContractClassifierRoute extends RestRouteBuilder {

	@Override
	public void configure() throws Exception {
		
	    rest("/getClassifierByContract")
	    	.post("/")
	    	.description("Servicio para invocar a WS SOAP GetContractClassifier")
	    	.type(DataInput.class)
	    	.outType(String.class)
	    	.consumes("application/json")
	    	.produces("text/plain")
	    	.to("direct:GetContractClassifier");	
	    
	    from("direct:GetContractClassifier")
	    	.handleFault()	    	
		    ...
		    .unmarshal().json(JsonLibrary.Jackson, DataInput.class)
		    .to("velocity:GetContractClassifier_Req.vm")
		    .log("HEADER IDA -> ${in.header.CamelSpringWebServiceSoapHeader}")
		    .log("BODY IDA -> ${body}")
		    .log("DESTINO: -> {{soap.remote.GetClassifierByContract.ep}} ACCION: -> {{soap.remote.GetClassifierByContract.action}} ")
	    	.to("spring-ws:{{soap.remote.GetClassifierByContract.ep}}?soapAction={{soap.remote.GetClassifierByContract.action}}")
	    	...
	    	.to("xslt:GetContractClassifier_Body_Rsp.xsl") //Por defecto, output es string
	    	.log("BODY VUELTA POST XSLT-> ${body}")  	
	    	
	    	.convertBodyTo(String.class)
	    	.to("xslt:GetContractClassifier_Aux_Rsp.xsl")
